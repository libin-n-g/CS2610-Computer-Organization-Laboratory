...
mov eax,0x0;;;;;;;;;;;loop for giving input;;;;;
L1:
	cmp eax,0xfa0
	jz exit
	mov ebx,0x1c
...
exit:
;=================================================================
;loop for processing input 
;Input Data at memeory location 0xb000 to 0xbfa0(not including 0xbfa0)
;==================================================================
mov cl,0x0;i=0
mov ch,0x0;j=0

mov eax,0x0
mov [0xfa0],eax;for average1
mov [0xfa4],eax;for average2
mov [0xfa8],eax;for average3
mov [0xfac],eax;for average4
mov [0xfb0],eax;for average5
mov eax,0xFA0;end of data
mov [0xfe0],eax;storing pointer to end of data
mov esi,0x0
...
istart:
	...
	jz ifinish; 
	mov ch,0x0;j=0
	mov [0xfc0],esi;storing pointer to start of structure array
	jstart:		
		mov eax,[esi];Q1
		add esi,0x4
		mov ebx,[esi];Q2
		...
		add esi,0x4
		mov [esi],eax;storing sum at Total
		add esi,0x44;for diff person same course
		mov [0xfd0],esi;current position
		mov esi,[0xfe0];pointer to course sum variable
		add eax,[esi];
		mov [esi],eax;adding current total to sum 
		mov [0xfe0],esi;
		mov esi,[0xfd0];reseting current position
		inc ch;j++
		cmp ch,0x32;j<50
		jnz jstart
	mov esi,[0xfe0]
	...
	mov [esi],eax
	add esi,0x4
	mov [0xfe0],esi;storing pointer for next course sum
	mov esi,[0xfc0]
	add esi,0x10
	inc cl
	jmp istart
...
dw 0x0000	;Limit 15->0 800
dw 0x0000	;Base 15->0
db 0		;Base 23->16
db 00000000b;P=1 DPL=0 S=1 Type=0000(Read only)
